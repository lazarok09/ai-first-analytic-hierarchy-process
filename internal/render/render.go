...
.legend-swatches .sw.eq{background:#f3efe6}
.legend-swatches .sw.hi.s5{background:rgba(15,122,95,.28)}
.legend-swatches .sw.hi.s9{background:rgba(15,122,95,.72);box-shadow:0 0 10px var(--glow-hi)}
.legend-swatches .sw.lo.s5{background:rgba(185,72,58,.28)}
.legend-swatches .sw.lo.s9{background:rgba(185,72,58,.72);box-shadow:0 0 10px var(--glow-lo)}
...
.matrix-expand{
  ...
  padding:.22rem .7rem;border-radius:999px;border:1px solid color-mix(in srgb,var(--accent) 45%,var(--line));
  background:var(--accent-soft);color:var(--accent)
}
.matrix-expand:hover{filter:brightness(0.97)}
.matrix-expand:focus-visible{outline:2px solid var(--accent);outline-offset:2px}
...
table.matrix{width:100%;border-collapse:separate;border-spacing:0;font-size:.95rem;background:#faf7f0;font-family:var(--font-mono)}
table.matrix th,table.matrix td{border:1px solid #e4ddd0;padding:.4rem .35rem;min-width:2.35rem;text-align:center}
table.matrix thead th,table.matrix tbody th{
  background:#f0ebe1;font-size:.78rem;font-weight:600;color:#3d3830;font-family:var(--font-sans);
...
table.matrix thead th{text-align:center;position:sticky;top:0;z-index:2;left:auto;max-width:none}
table.matrix th.corner{left:0;z-index:3;background:#e8e2d6}
table.matrix td span{display:inline-block;min-width:1.35rem;font-variant-numeric:tabular-nums;font-weight:600}
table.matrix td.diag{background:var(--diag);color:var(--muted);box-shadow:inset 0 0 0 1px #ddd5c6}
table.matrix td.eq{background:#f5f1e8;color:#4a453c}
table.matrix td.hi.s2{background:rgba(15,122,95,.10)}
table.matrix td.hi.s3{background:rgba(15,122,95,.16)}
table.matrix td.hi.s4{background:rgba(15,122,95,.24)}
table.matrix td.hi.s5{background:rgba(15,122,95,.34);box-shadow:inset 0 0 12px rgba(15,122,95,.12)}
table.matrix td.hi.s6{background:rgba(15,122,95,.44);color:#063528;box-shadow:0 0 10px var(--glow-hi)}
table.matrix td.hi.s7{background:rgba(15,122,95,.56);color:#04281e;box-shadow:0 0 14px var(--glow-hi)}
table.matrix td.hi.s8,table.matrix td.hi.s9{background:rgba(15,122,95,.70);color:#021a14;box-shadow:0 0 18px var(--glow-hi)}
table.matrix td.lo.s2{background:rgba(185,72,58,.10)}
table.matrix td.lo.s3{background:rgba(185,72,58,.16)}
table.matrix td.lo.s4{background:rgba(185,72,58,.24)}
table.matrix td.lo.s5{background:rgba(185,72,58,.34);box-shadow:inset 0 0 12px rgba(185,72,58,.12)}
table.matrix td.lo.s6{background:rgba(185,72,58,.44);color:#4a1510;box-shadow:0 0 10px var(--glow-lo)}
table.matrix td.lo.s7{background:rgba(185,72,58,.56);color:#3a100c;box-shadow:0 0 14px var(--glow-lo)}
table.matrix td.lo.s8,table.matrix td.lo.s9{background:rgba(185,72,58,.70);color:#2a0b08;box-shadow:0 0 18px var(--glow-lo)}
...
/* Generic TOC target flash — any #id the contents nav links to */
[data-flash]{
  border:2px solid transparent;
  border-radius:12px;
  animation:toc-border-blink .5s ease-in-out 4;
}
@keyframes toc-border-blink{
...
// reportTOCJS flashes whatever element a Contents link targets (any report page / section set).
const reportTOCJS = `(function(){
  var FLASH_MS=2000,timer=0;
  function flash(el){
    if(!el)return;
    el.removeAttribute("data-flash");
    void el.offsetWidth;
    el.setAttribute("data-flash",String(Date.now()));
    clearTimeout(timer);
    timer=setTimeout(function(){el.removeAttribute("data-flash");},FLASH_MS);
  }
  document.addEventListener("click",function(e){
    var a=e.target.closest&&e.target.closest('nav.toc a[href^="#"]');
    if(!a)return;
    ...
    if(!el)return;
    requestAnimationFrame(function(){flash(el);});
  });
})();`
...
func writeMatrixCard(b *strings.Builder, key string, result *workspace.ComputeResult) {
	...
	fmt.Fprintf(b, `<dialog id="%s" class="matrix-dialog" aria-labelledby="%s-dialog-title">`, html.EscapeString(dialogID), mid)
